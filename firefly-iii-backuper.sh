...
# url: https://gist.github.com/WaleedMortaja/ddc87fc4dc7dcb73178e971c299d7e78

files_to_backup=(.env .db.env .fidi.env .importer.env docker-compose.yml docker-compose-importer.yml)
...
intro () {
    echo " ====================================================="
    echo "     Backup & Restore docker based FireFly III v1.2-3 "
    echo " ====================================================="
    echo " It automatically detects db & upload volumes based on the name firefly[_-]iii"
    echo " Requirements:"
...
backup () {
    ...
    # Version
    app_container=$(docker ps | grep -F 'firefly_iii_core' | cut -d ' ' -f 1)
    if [ "$app_container" == "" ]; then
        fatal "app container is not running. Not backing up."
    fi
    ...
    # DB container
    db_container=$(docker ps | grep -F 'firefly_iii_db' | cut -d ' ' -f 1)
